...
function renderKPIs(t,days){
  const base=activeMetaBase(); const scale=days/30;
...
function renderDaily(rows,days){
  const base=activeMetaBase(); const metaDia=base.leads/30;
  const useG=state.platform!=='facebook', useF=state.platform!=='google';
...
function setStatus(source){
  ...
  else { tag.textContent='modo demo'; }
}
...
async function boot(){
  ...
  DATA=res.rows; LAST=DATA[DATA.length-1].dt;
